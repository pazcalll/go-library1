...
func Init() *echo.Echo {
	...
	e.GET("/stock", admin.GetStock)                // mendapatkan stok semua buku, tapi akan mendapat stok dari sebuah buku jika diberi book_id (int) is given sebagai parameter
	...
	e.POST("/upload/book", admin.BookUpload)       // menambahkan buku dengan name (string), author (string), img (file: .png, .jpg, .jpeg) sebagai param
	...
	e.GET("/return-detail", admin.GetReturnDetail) // mendapat informasi detail dari buku yang dipinjam dengan parameter id
